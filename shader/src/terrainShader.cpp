...
void Shader::TerrainShader::render(GLuint VAO, std::shared_ptr<Camera::CameraIf>& m_cameraIf, glm::mat4& m_modelMatrix, const GPUObject::TextureStructure textureStruct, GLuint numInd)
{
	...
	glm::vec3 lightPositionTerrain(380.0f, 27.0f, 380.0f);
	glUniform3f(m_lightPositionID, lightPositionTerrain[0], lightPositionTerrain[1], lightPositionTerrain[2]);
	...
	glBindTexture(GL_TEXTURE_2D, textureStruct.m_texture1);

	glUniform1i(m_gTextureID, 2);
	glActiveTexture(GL_TEXTURE2);
	glBindTexture(GL_TEXTURE_2D, textureStruct.m_texture2);

	glUniform1i(m_bTextureID, 3);
	glActiveTexture(GL_TEXTURE3);
	glBindTexture(GL_TEXTURE_2D, textureStruct.m_texture3);

	glUniform1i(m_blendMapID, 4);
	glActiveTexture(GL_TEXTURE4);
	glBindTexture(GL_TEXTURE_2D, textureStruct.m_texture4);
